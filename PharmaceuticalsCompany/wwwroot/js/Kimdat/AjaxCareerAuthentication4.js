...
$(document).ready(function () {
    ...
    $("#register").submit(function (e) {
        ...
        var fullName = $('#register input[name="FullName"]').val().replace(/\s/g, '');
        if (fullName != "") {
            ...
            if (fullName.length < 6 || fullName.length > 50) {
                ...
                $('#register input[name="FullName"]').parent().find(".error").html("full name must be valid(only alpha characters and space)");
                flagPersonal = false;
            }
        }
        ...
        $(".name_school").each(function () {
            ...
            else if (!regexName.test($(this).val().replace(/\s/g, ''))) {
        ...
        if ($('input[name="file"]').val() == ""&& $('.filename').html()=="") {
