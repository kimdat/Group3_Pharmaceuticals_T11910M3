...
function Validation() {
    ...
    $(".name_school").each(function () {
        ...
        else if ($(this).val().replace(/\s/g, '').length < 6 || $(this).val().replace(/\s/g, '').length > 100) {
        ...
        else if (!regexNameSchool.test($(this).val().replace(/\s/g, ''))) {
...
function newwin() {
    ...
    if (valid == true) {
        ...
        if (first < 11) {
            ...
            $(".modalValid").click();
            $("#ModalValid .modal-body p").html("<br/>You Can just add only lower than 10 schools");
        }
...
function ValidationPersonal() {
    ...
    if (!regexName.test($(name).val().replace(/\s/g, '')) )
    {
...
var $dropzonePhoto = $('.dropzonePhoto');
...
$(document).ready(function () {
    ...
    $dropzonePhoto.on('drag dragstart dragend dragover dragenter dragleave drop', function (e) {

       
    })
        .on('dragover dragenter', function () {
            $dropzonePhoto.addClass('is-dragover');
        })
        .on('dragleave dragend drop', function () {
            $dropzonePhoto.removeClass('is-dragover');
        })
